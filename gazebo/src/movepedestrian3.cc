...
namespace gazebo
{
  class MovePedestrian3 : public ModelPlugin
  {
    ...
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
    {
      ...
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          std::bind(&MovePedestrian3::OnUpdate, this));
      ...
      if (!ros::isInitialized())
      {
        int argc = 0;
        char **argv = NULL;
        ros::init(argc, argv, "move_pedestrian3",
            ros::init_options::NoSigintHandler);
      }
         
      // Create our ROS node. This acts in a similar manner to
      // the Gazebo node
      this->rosNode.reset(new ros::NodeHandle("move_pedestrian3"));
      ...
      ros::SubscribeOptions so =
        ros::SubscribeOptions::create<std_msgs::Float32MultiArray>(
            "/pedestrian3_set_vel",
            1,
            boost::bind(&MovePedestrian3::SetVelocity, this, _1),
            ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);
      
      // Spin up the queue helper thread.
      this->rosQueueThread =
        std::thread(std::bind(&MovePedestrian3::QueueThread, this));
         
      ROS_WARN("Loaded MovePedestrian3 Plugin with parent...%s", this->pedestrian->GetName().c_str());
  ...
  GZ_REGISTER_MODEL_PLUGIN(MovePedestrian3)
}
