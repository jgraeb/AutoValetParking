...
class RRTStarReedsShepp(RRTStar):
    ...
    def __init__(self, start, goal, obstacle_list, rand_areax,rand_areay,max_iter=100,connect_circle_dist=50.0):
        """
        Setting Parameter
        ...
        self.min_randx = rand_areax[0]
        self.min_randy = rand_areay[0]
        self.max_randx = rand_areax[1]
        self.max_randy = rand_areay[1]
...
def mainAVP(obstacleList, max_iter=100): # not using this
    ...
    # # Set Initial parameters
    start = [50.0, 2.0, np.deg2rad(90.0)]
    goal = [29.0, 17.0, np.deg2rad(90.0)]
    #oal = [3.17, 5.31, np.deg2rad(115.0)]
        # Set Initial parameters
    # start = [10.0, 0.0, np.deg2rad(0.0)]
    # goal = [6.0, 7.0, np.deg2rad(90.0)]
    # intermediate point 290,170
    rrt_star_reeds_shepp = RRTStarReedsShepp(start, goal,obstacleList,[0.0, 55.0], max_iter=max_iter)#[956, 768]
    path = rrt_star_reeds_shepp.planning(animation=show_animation)
...
def mainAVPtest(obstacleList,start,goal,rand_areax,rand_areay, max_iter=100): # using this
    ...
    # goal = [6.0, 7.0, np.deg2rad(90.0)]
    # intermediate point 290,170
    rrt_star_reeds_shepp = RRTStarReedsShepp(start, goal,obstacleList,rand_areax,rand_areay, max_iter=max_iter)#[956, 768]
    path = rrt_star_reeds_shepp.planning(animation=show_animation)
